...
StaticPopupDialogs["DYNAMICCAM_NO_DLL"] = {
    text = "The ConsoleXP library is not loaded into the client and without it the DynamicCam AddOn cannot work, paste the following link into your browser for more information.",
    button1 = "Close", 
    showAlert = true,
		timeout = 0,
		whileDead = true,
		...
		OnShow = function(self, data)
				self.editBox:SetText("https://github.com/leoaviana/ConsoleXP")
...
if(not C_ConsoleXP) then 
  StaticPopup_Show("DYNAMICCAM_NO_DLL")  
  DynamicCam.dllNotLoaded = true
  return;
end
...
function DynamicCam:DC_SetCVar(cvar, setting)
  ...
  if(cvar == "cameraDistanceMaxZoomFactor") then
...
function DynamicCam:OpenMenu()
  ...
  if(not Addon.OptionsFrame) then
    Addon.OptionsFrame = AceGUI:Create("Frame")
  end

  LibStub("AceConfigDialog-3.0"):Open("DynamicCam", Addon.OptionsFrame)
...
function DynamicCam:ZoomSlash(input)
  ...
  if zoom and (zoom <= DynamicCam.cameraDistanceMaxZoomFactor_max or zoom >= 0) then
    local defaultTime = math.abs(zoom - GetCameraZoom()) / tonumber(GetCVar("cameraDistanceMoveSpeed"))
