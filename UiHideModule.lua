...
Addon.C_Timer.DC_TimerAfterFrame = nil
Addon.C_Timer.DC_TimerAfterTable = {};

Addon.C_Timer.After = function(delay, func, ...)
	if(type(delay)~="number" or type(func)~="function") then
	  return false;
	end
	if (Addon.C_Timer.DC_TimerAfterFrame == nil) then
	  Addon.C_Timer.DC_TimerAfterFrame = CreateFrame("Frame","DC_TimerAfterFrame", UIParent);
	  Addon.C_Timer.DC_TimerAfterFrame:SetScript("onUpdate",function (self,elapse)
		local count = #Addon.C_Timer.DC_TimerAfterTable;
		local i = 1;
		while(i<=count) do
		  local waitRecord = tremove(Addon.C_Timer.DC_TimerAfterTable,i);
		  local d = tremove(waitRecord,1);
		  local f = tremove(waitRecord,1);
		  local p = tremove(waitRecord,1);
		  if(d>elapse) then
			tinsert(Addon.C_Timer.DC_TimerAfterTable,i,{d-elapse,f,p});
			i = i + 1;
		  else
			count = count - 1;
			f(unpack(p));
	...
	tinsert(Addon.C_Timer.DC_TimerAfterTable,{delay,func,{...}});
	return true;
end 
...
local function IsFontOrTexture(obj)
        return obj:GetObjectType() == "FontString" or obj:GetObjectType() == "Texture"
end
...
local function SetIgnoreParentAlpha(frame, ignore)
  ...
  if ignore then
    if not frame.IgnoringParentAlpha then
      ...
      if(not IsFontOrTexture(frame)) then
        frame:SetParent(nil)
      end
    ...
    if frame.IgnoringParentAlpha then
      if frame.IgnoringParentAlpha.Parent then
        frame:SetParent(frame.IgnoringParentAlpha.Parent)
        SafeSetFrameLevel(frame, frame.IgnoringParentAlpha.Level)
        SafeSetFrameStrata(frame, frame.IgnoringParentAlpha.Strata)
      end
...
local function ConditionalSetIgnoreParentAlpha(frame, ignoreParentAlpha)
  ...
  if frame.ludius_ignoreParentAlphaBeforeFadeOut == nil then
    frame.ludius_ignoreParentAlphaBeforeFadeOut = frame.IgnoringParentAlpha ~= nil
  end

  if (frame.IgnoringParentAlpha ~= nil) ~= ignoreParentAlpha then
...
local function ConditionalResetIgnoreParentAlpha(frame)
  ...
  if (frame.IgnoringParentAlpha ~= nil) ~= frame.ludius_ignoreParentAlphaBeforeFadeOut then
    frame.ludius_intendedIgnoreParentAlpha = frame.ludius_ignoreParentAlphaBeforeFadeOut
    SetIgnoreParentAlpha(frame, frame.ludius_ignoreParentAlphaBeforeFadeOut)
  end
...
local function CollectAlertFrame(_, frame)
  ...
  if currentAlertFramesIgnoreParentAlpha and not (frame.IgnoringParentAlpha ~= nil) then
...
local function FadeOutFrame(frame, duration, targetIgnoreParentAlpha, targetAlpha)
  ...
  if targetIgnoreParentAlpha then
    ...
    if not (frame.IgnoringParentAlpha ~= nil) then
  ...
  -- Frame should henceforth adhere to parent alpha.
    ...
    -- parent alpha.
    if frame.IgnoringParentAlpha ~= nil then
