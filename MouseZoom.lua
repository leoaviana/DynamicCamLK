...
local function GetCameraZoom()
  return LibCamera:GetCameraZoom()
end
...
local function SetMinZoom(zoom)
  ...
  minZoomValues[DynamicCam.modelFrame:GetModel()] = zoom
end
...
local function GetMinZoom()
  if IsMounted() then
    ...
    DynamicCam.modelFrame:SetUnit("player")
    return minZoomValues[DynamicCam.modelFrame:GetModel()] or 1.5
  end
end
  ...
  if increments == 1 then
    ...
    -- zoom out
      ...
      if currentZoom == 0 then
        ...
        reactiveZoomTarget = math.min(GetCVar("cameraDistanceMaxFactor")*15, reactiveZoomTarget + increments)
      end
    end
    ...
    local zoomTime = math.min(maxZoomTime, math.abs(reactiveZoomTarget - currentZoom) / tonumber(GetCVar("cameraDistanceMoveSpeed")) )
...
function DynamicCam:ToggleRZVA()
  ...
  if not rzvaFrame then
    ...
    rzvaFrame.t:SetVertexColor(1, 1, 1, 0.1) -- Apply the color and transparency
    ...
    rzvaFrame.cameraZoomLabel:SetText("|cFFFF4D4D" .. L["Actual\nZoom\nValue"] .. "|r")
    ...
    rzvaFrame.rzi.t:SetVertexColor(1, 1, 0, 1)
