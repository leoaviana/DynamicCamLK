...
local function SetIgnoreParentAlpha(frame, ignore, AceGUIFrame)
  if not frame then return end
  if ignore then
    if not frame.IgnoringParentAlpha then
      frame.IgnoringParentAlpha = {
      ...
      SafeSetParent(frame, nil) 
      if(AceGUIFrame) then
        AceGUIFrame.parent = nil
      end
      SafeSetFrameLevel(frame, frame.IgnoringParentAlpha.Level)
      SafeSetFrameStrata(frame, frame.IgnoringParentAlpha.Strata)
    end
  else
    if frame.IgnoringParentAlpha then
      if frame.IgnoringParentAlpha.Parent then
        if(AceGUIFrame) then 
          SafeSetParent(frame, nil)
        end
        SafeSetParent(frame, frame.IgnoringParentAlpha.Parent)
        if(AceGUIFrame) then
          AceGUIFrame.parent = frame.IgnoringParentAlpha.Parent 
        end
        SafeSetFrameLevel(frame, frame.IgnoringParentAlpha.Level)
        SafeSetFrameStrata(frame, frame.IgnoringParentAlpha.Strata)
      end
      frame.IgnoringParentAlpha = nil
    end
  end
end
...
function DynamicCam:SettingsPanelSetIgnoreParentAlpha(ignoreParentAlpha) 
    SetIgnoreParentAlpha(GameMenuFrame, ignoreParentAlpha)
    SetIgnoreParentAlpha(InterfaceOptionsFrame, ignoreParentAlpha)
    
    if(Addon.OptionsFrame) then
      SetIgnoreParentAlpha(Addon.OptionsFrame.frame, ignoreParentAlpha, Addon.OptionsFrame) 
    end

    for i = 1, LibStub("AceGUI-3.0"):GetNextWidgetNum("Dropdown-Pullout") do
        local pullout = _G["AceGUI30Pullout" .. i]
        if pullout then
            SetIgnoreParentAlpha(pullout, ignoreParentAlpha)
        end
    end
end
...
local pitchGroupVars = {
  ...
  {"cvars", "test_cameraDynamicPitchHeight"},
  {"cvars", "test_cameraDynamicPitchHeightFlying"},
          ...
          pitchSubGroup = {
            ...
            args = {
              ...
              basePitchHeight = {
                ...
                get =
                  function()
                    return DynamicCam:GetSettingsValue(forSituations and SID, "cvars", "test_cameraDynamicPitchHeight")
                  end,
                set =
                  function(_, newValue)
                    DynamicCam:SetSettingsValue(newValue, forSituations and SID, "cvars", "test_cameraDynamicPitchHeight")
                  end,
              },
              baseFovPadFlyingReset =
                CreateSliderResetButton(3.1, forSituations, "cvars", "test_cameraDynamicPitchHeight"),
              blank3 = {type = "description", name = "\n\n", order = 3.2, },
              ...
              baseFovPadFlyingReset =
                CreateSliderResetButton(4.1, forSituations, "cvars", "test_cameraDynamicPitchBaseFovPadFlying"),
              blank3 = {type = "description", name = "\n\n", order = 4.2, },

              basePitchHeightFlying = {
                type = "range",
                name = "Camera Height (flying)",
                order = 5,
                width = sliderWidth,
                desc = "|cff909090cvar: test_cameraDynamicPitch\nHeightFlying|r",
                min = 0,
                ...
                get =
                  function()
                    return DynamicCam:GetSettingsValue(forSituations and SID, "cvars", "test_cameraDynamicPitchHeightFlying")
                  end,
                set =
                  function(_, newValue)
                    DynamicCam:SetSettingsValue(newValue, forSituations and SID, "cvars", "test_cameraDynamicPitchHeightFlying")
                  end,
              },
              baseFovPadFlyingReset =
                CreateSliderResetButton(5.1, forSituations, "cvars", "test_cameraDynamicPitchHeightFlying"),
              blank3 = {type = "description", name = "\n\n", order = 5.2, },
--[==[
              baseFovPadDownScale = {
                type = "range",
...
--[==[
              ...
              rangeScaleReset =
                CreateSliderResetButton(6.1, forSituations, "cvars", "test_cameraHeadMovementRangeScale",
              ...
              hideUIHelpGroup = {
                ...
                args = {
                  ...
                  settingsPanelIgnoreParentAlpha = {
                    ...
                    get =
                      function()
                        return InterfaceOptionsFrame.IgnoringParentAlpha ~= nil
                      end,
                    set =
                      function(_, newValue)
...
function Options:RegisterMenus()
  ...
  local function AddOpenButtonToOptions()
  local button = CreateFrame("Button", nil, self.menu, "UIPanelButtonTemplate")
  button:SetSize(150, 22) -- Button size
  button:SetText("Expand Window") -- Button text
  button:SetPoint("TOPRIGHT", self.menu, "TOPRIGHT", -20, -20) -- Position: top-right with padding
  
  button:SetScript("OnEnter", function(self)
      GameTooltip:SetOwner(self, "ANCHOR_TOPRIGHT")
      GameTooltip:SetText("Click to open a resizable window for DynamicCam settings.", 1, 0.82, 0, true)
      GameTooltip:Show()
  end)
...
-- Attach the button when the panel is shown
self.menu:HookScript("OnShow", function()
    if not self.menu.hasButton then
        AddOpenButtonToOptions()
        self.menu.hasButton = true -- Ensure the button is only created once
    end
end)
...
line:SetTexture("Interface/Buttons/WHITE8X8") -- Use a white texture
line:SetVertexColor(r, g, b, a) -- Set the color and transparency
line:SetPoint("TOPLEFT", f, startRelativeAnchor, startOffsetX, startOffsetY)

-- Calculate width, height, and angle
local width = math.sqrt((endOffsetX - startOffsetX)^2 + (endOffsetY - startOffsetY)^2)
local angle = math.atan2(endOffsetY - startOffsetY, endOffsetX - startOffsetX)

-- Set the line's size and rotation
line:SetWidth(width)
line:SetHeight(thickness)
line:SetTexCoord(0, 1, 0, 1) -- Optional for texture alignment
